...
const Login = () => {
  ...
  const validar = () => {
    ...
    else if (state.userName !== localStorage.getItem("user"))
      errorList = { ...errorList, user: "Usuario o Contraseña no registrado." };
    if (!state.password)
      errorList = { ...errorList, password: "Campo obligatorio" };
    else if (state.password !== localStorage.getItem("password"))
      errorList = {
        ...errorList,
        password: "Usuario o Contraseña no registrado.",
      };
  ...
  return (
    <div className="login bg-dark pt-5">
      <Link to="/">
      ...
      <form
        action=""
        className="mt-5 d-flex flex-column justify-content-around gap-1 w-75 mx-auto"
      ...
      >
        ...
        <button type="submit" className="btn mx-auto w-75">
          Ingresar
        </button>
      </form>
    </div>
